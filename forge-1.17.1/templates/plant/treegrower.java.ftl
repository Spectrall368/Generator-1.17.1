...
public class ${name}TreeGrower extends <#if (data.megaTrees[0]?has_content) || (data.megaTrees[1]?has_content)>AbstractMegaTreeGrower<#else>AbstractTreeGrower</#if> {
	protected ConfiguredFeature<TreeConfiguration, ?> getConfiguredFeature(Random randomSource, boolean hasFlower) {
	...
	protected ConfiguredFeature<TreeConfiguration, ?> getConfiguredMegaFeature(Random randomSource) {
...
<#if tree?has_content>
ServerLifecycleHooks.getCurrentServer().registryAccess().registryOrThrow(Registry.CONFIGURED_FEATURE_REGISTRY).get(ResourceKey.create(Registry.CONFIGURED_FEATURE_REGISTRY, new ResourceLocation("${generator.map(tree, "configuredfeatures")}")))
<#else>
null
</#if>
</#macro>