...
@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD) public class ${name}BrewingRecipe implements IBrewingRecipe {
	...
	public boolean isInput(ItemStack input) {
		...
		return Ingredient.of(ItemTags.getAllTags().getTagOrEmpty(new ResourceLocation("${data.brewingInputStack?replace("TAG:","")}"))).test(input);
		<#else>
	...
	public boolean isIngredient(ItemStack ingredient) {
		...
		return Ingredient.of(ItemTags.getAllTags().getTagOrEmpty(new ResourceLocation("${data.brewingIngredientStack?replace("TAG:","")}"))).test(ingredient);
		<#else>
