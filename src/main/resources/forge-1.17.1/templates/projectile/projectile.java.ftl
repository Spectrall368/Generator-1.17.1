...
public class ${name}Entity extends AbstractArrow implements ItemSupplier {
	...
	public static ${name}Entity shoot(Level world, LivingEntity entity, Random source) {
		return shoot(world, entity, source, ${data.power}f, ${data.damage}, ${data.knockback});
	}

	public static ${name}Entity shoot(Level world, LivingEntity entity, Random source, float pullingPower) {
